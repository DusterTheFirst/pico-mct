...
declare interface Clock {
    ...
    /**
     * A function supporting registration of a new callback that will be invoked when the clock next
     * ticks. It will be invoked with two arguments:
     * @param event A string specifying the event to listen on. For now, clocks support one event - tick.
     * @param callback A function that will be invoked when this clock ticks. The function must be
     * invoked with one parameter - a number representing a valid time in the current time system.
     */
    on(event: "tick", callback: (time: number) => void): void;

    /** A function that allows de-registration of a tick listener. It accepts the same arguments as on. */
    off(event: "tick", callback: (time: number) => void): void;
}
//#endregion
...
declare module plugins {
    ...
    function Conductor(options: {
        menuOptions: (
            | { timeSystem: string; clock: string; clockOffsets: ClockOffsets }
            | {
                  timeSystem: string;
                  bounds: TimeBounds;
                  zoomOutLimit?: number;
