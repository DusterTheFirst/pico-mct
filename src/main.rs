...
use telemetry::{get_telemetry_values, TelemetryValue};
use tide::{
    http::headers::HeaderValue, security::CorsMiddleware, Body, Response, ResponseBuilder,
    StatusCode,
};
...
async fn main() -> color_eyre::Result<()> {
    ...
    app.at("/history/:key");
    app.at("/measurements")
        .get(|req| async move { Body::from_json(&get_telemetry_values()) });
